CREATE OR REPLACE PACKAGE BODY PAQUETE_TRABAJADOR2 IS
  ...
  PROCEDURE visualizar_lista_tipoTrabaj
  (
    
    pCategoria TRABAJADORES.CATEGORIA%TYPE, un_cursor OUT TRABAJADOR_CURSOR
  )
  IS
  BEGIN
    OPEN un_cursor FOR
      select dni, categoria
      from trabajadores
      where categoria=pCategoria; 
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      RAISE_APPLICATION_ERROR(-20107, 'NO SE ENCONTRARON TRABAJADORES');
  END visualizar_lista_tipoTrabaj;
  ...
  BEGIN  
     OPEN un_cursor FOR 
        SELECT dni, jefe, nombre, apellidouno, apellidodos, calle, portal, piso, mano, telefempre, telefperso, salario, fecha_nac, categoria, centro, usuario
        FROM TRABAJADORES
        WHERE dni = pDni;
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
  ...
  BEGIN
    OPEN un_cursor FOR 
      SELECT nombre FROM TRABAJADORES WHERE dni=pDni;
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      RAISE_APPLICATION_ERROR(-20009,'NO SE ENCONTRÓ EL TRABAJADOR');
